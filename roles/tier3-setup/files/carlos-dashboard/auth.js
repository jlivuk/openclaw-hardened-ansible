...
const ADMIN_DB_PATH = path.join(DATA_DIR, 'carlos-admin.db');
...
function migrateFromJson() {
  // Check if admin DB already has users — if so, migration already done
  ...
  for (const [username, user] of Object.entries(users)) {
    ...
    // john gets admin role, everyone else gets user
...
function verifyPassword(password, storedHash, storedSalt) {
  ...
  // Legacy SHA-256 hash (backward compat)
...
function authenticateRequest(req) {
  // Check API key first (for health-sync)
  ...
  // Pull fresh role from DB (in case it changed since token was issued)
